    ...
    cols: [
     ...
     },
	 {
	   ...
	   },
	   {
	     ...
	     width: 100,
	     templet: function (d) { 
	   	    if(d.status==1){return '发起支付'}
	   	    else if(d.status==2){return '预支付成功'}
			else if(d.status==3){return '预支付失败'} 
			else if(d.status==4){return '付款成功'} 
			else if(d.status==5){return '付款失败'} 
			else if(d.status==6){return '撤销订单'}		  
	     },
	   },
	   {
	     field: "totalFee",
